...
async function setupAndSeed() {
  try {
    ...
    const tables = createTables.split(';').filter(stmt => stmt.trim());
    ...
    for (const product of products) {
      ...
      console.log(`  ✓ Added: ${product.name.substring(0, 50)}...`);
    }

    console.log(`\n🎉 Setup completed successfully! Added ${products.length} products.`);
    console.log('🌐 Your products should now be visible on the website!');
    process.exit(0);
  } catch (error) {
